...
if (Z_SingleCurrency) then {
	_formattedText = format [
		...
		"<t color='#33BFFF' size='0.7'>%12: </t><t color='#ffffff' size='0.7'>%5 %27</t><br />" +
		...
		if (_sellPrice >= 9999999) then {"UNBUYABLE HERE";} else { if (_sellPrice >= 0) then {[_sellPrice] call BIS_fnc_numberText;} else {"";};},
		if (_buyPrice >= 9999999) then {"UNBUYABLE HERE";} else {if (_buyPrice >= 0) then {[_buyPrice] call BIS_fnc_numberText;} else {"";};},
		//CurrencyName,
		...
		if (_sellPrice >= 9999999)  then {"";} else {CurrencyName;}	// #26
	];
} else {
	_picSell = "";
