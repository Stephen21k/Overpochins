if(isServer) then {
	...
	[_mission,_position,"Hard","ARMY Base","MainHero",true] call mission_init;
	
	diag_log 		format["WAI: [Mission:[Hero] ARMY Base]: Starting... %1",_position];
...
_baserunover42 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 0.01, (_position select 1) - 0.01,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 12, (_position select 1) + 2, 0],4,"Hard",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 2, (_position select 1) + 2, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 14, (_position select 1) - 35, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 13, (_position select 1) + 35, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;

	//Humvee Patrol
	[[(_position select 0) - 22, (_position select 1) - 56, 0],[(_position select 0) + 22, (_position select 1) + 56, 0],50,2,"HMMWV_Armored","Hard","Bandit","Bandit",_mission] call vehicle_patrol;
	...
	[[[(_position select 0) + 0.1, (_position select 1) - 25, 0]],"M2StaticMG","Hard","Bandit","Bandit",0,2,"Random","Random",_mission] call spawn_static;
	...
	if(_complete) then {
		[_crate,10,5,10,5,2] call dynamic_crate;
			};



	diag_log format["WAI: [Mission:[Hero] ARMY Base]: Ended at %1",_position];
