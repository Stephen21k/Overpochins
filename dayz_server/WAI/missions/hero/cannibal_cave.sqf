if(isServer) then {
	...
	[_mission,_position,"hard","Cannibal Cave","MainHero",true] call mission_init;
...
_baserunover23 = createVehicle ["UralWreck", [(_position select 0) - 41.3, (_position select 1) - 26,-0.02],[], 0, "CAN_COLLIDE"];
	...
	[[(_position select 0) + 12, (_position select 1) + 42.5, 0],_num,"extreme",["random","at"],4,"random","TK_INS_Soldier_AT_EP1","random",["bandit",150],_mission] call spawn_group;
	[[(_position select 0) + 11, (_position select 1) + 41, 0],4,"hard","random",4,"random","TK_Special_Forces_EP1","random","bandit",_mission] call spawn_group;
	[[(_position select 0) - 12, (_position select 1) - 43, 0],4,"random","random",4,"random","TK_Special_Forces_EP1","random","bandit",_mission] call spawn_group;
	[[(_position select 0) - 13, (_position select 1) - 43, 0],4,"random","random",4,"random","TK_INS_Soldier_AT_EP1","random","bandit",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,16,8,[3,crate_items_high_value],3,[4,crate_backpacks_large]] call dynamic_crate;
	};	
	
	// End of mission
	diag_log format["WAI: Mission:[Hero] Cannibal ended at %1 ended",_position];
