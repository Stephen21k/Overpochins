if(isServer) then {
	...
	[_mission,_position,"Hard","Junk Yard","MainHero",true] call mission_init;
...
_baserunover31 = createVehicle ["Land_Fire_barrel_burning", [(_position select 0) + 2, (_position select 1) - 9,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 2, (_position select 1) - 5, 0],4,"Hard",["Random","AT"],4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) - 19, (_position select 1) + 19, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	[[(_position select 0) + 17, (_position select 1) + 21, 0],4,"Hard","Random",4,"Random","Bandit","Random","Bandit",_mission] call spawn_group;
	...
	_complete = [
		...
		"Bandits are hiding something in a junk yard...go check it out",	// mission announcement
	...
	if(_complete) then {
		[_crate,14,5,1,2,2] call dynamic_crate;
			};



	diag_log format["WAI: [Mission:[Hero] Junk Yard]: Ended at %1",_position];
