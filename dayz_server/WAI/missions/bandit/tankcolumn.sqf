if(isServer) then {
	...
	private 		["_complete","_crate_type","_mission","_position","_crate","_baserunover0","_baserunover1","_baserunover2","_baserunover3","_baserunover4","_baserunover5","_baserunover6","_baserunover7","_baserunover8","_baserunover9","_baserunover10","_baserunover11","_baserunover12","_baserunover13"];

	// Get mission number, important we do this early
	_mission 		= count wai_mission_data -1;

	_position		= [30] call find_position;
	[_mission,_position,"Hard","Tank Column","MainBandit",true] call mission_init;
...
_baserunover13 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 9, (_position select 1) - 1,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 7, (_position select 1) - 10, 0],4,"Hard",["Random","AT"],4,"Random","UN_CDF_Soldier_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 16, (_position select 1) - 5, 0],4,"Hard","Random",4,"Random","UN_CDF_Soldier_Guard_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 4, (_position select 1) + 18, 0],4,"Hard","Random",4,"Random","UN_CDF_Soldier_EP1","Random","Hero",_mission] call spawn_group;
	...
	_complete = [
		...
		"A U.N tank column stopped to resupply at an outpost...check your map",	// mission announcement
	...
	if(_complete) then {
		[_crate,12,5,30,5,2] call dynamic_crate;
			};

	diag_log format["WAI: [Mission:[Bandit] Tank Column]: Ended at %1",_position];
