if(isServer) then {
	...
	[_mission,_position,"Hard","Gem Tower","MainBandit",true] call mission_init;
...
_baserunover = [_baserunover0,_baserunover1,_baserunover2,_baserunover3,_baserunover4,_baserunover5];
	...
	[[(_position select 0) + 29, (_position select 1) - 21, 0],_num,"Hard",["Random","AT"],4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 21, (_position select 1) + 19, 0],4,"Hard","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 23, (_position select 1) - 19, 0],4,"Hard","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 12, (_position select 1) + 23, 0],4,"Hard","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 18, _position select 1, 0],4,"Hard","Random",4,"Random","Hero","Random","Hero",_mission] call spawn_group;

	//Humvee Patrol
[[(_position select 0) + 50, _position select 1, 0],[(_position select 0) - 60, _position select 1, 0],50,2,"HMMWV_Armored","Hard","Hero","Hero",_mission] call vehicle_patrol;
	 
	//Static Guns
[[[(_position select 0) - 1, (_position select 1) + 39, 0]],"KORD_high_TK_EP1","Easy","Hero","Hero",0,2,"Random","Random",_mission] call spawn_static;
[[[(_position select 0) + 33, (_position select 1) - 21, 0]],"KORD_high_TK_EP1","Easy","Hero","Hero",0,2,"Random","Random",_mission] call spawn_static;
	...
	if(_complete) then {
	[_crate,8,5,[5,crate_items_gems],3,2] call dynamic_crate;
	};	
