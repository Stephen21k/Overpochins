...
diag_log  format["WAI: [Mission:[Bandit] Hippy Commune]: Starting... %1",_position];
...
//Humvee Patrol
[[(_position select 0) + 55, _position select 1, 0],[(_position select 0) + 50, _position select 1, 0],50,2,"HMMWV_Armored","Hard","Hero","Hero",_mission] call vehicle_patrol;


//Static Guns
[[[(_position select 0) - 48, (_position select 1) + 0.1, 0]],"M2StaticMG","Hard","Policeman","Hero",0,2,"Random","Random",_mission] call spawn_static;
[[[(_position select 0) + 2, (_position select 1) +48, 0]],"M2StaticMG","Hard","Policeman","Hero",0,2,"Random","Random",_mission] call spawn_static;
...
"Survivors did not stop the bandits in time." // mission fail
  ...
  if(_complete) then {
  [_crate,6,5,[20,crate_items_crop_raider],3,3] call dynamic_crate;
};

diag_log format["WAI: [Mission:[Bandit] Hippy Commune]: Ended at %1",_position];
