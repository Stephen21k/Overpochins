...

{
	...
	*/
		...
		if(_type in DZE_Origins_Buildings) then {
			//origins get owner UID from worldspace
			_originowner = _worldspace select 4;
				diag_log format["Origins Object: %1 - %2", _type,_originowner];
				_object setVariable ["CanBeUpdated",false, true];
				{
				
					_object setVariable ["OwnerUID",_originowner, true];
